...
function initialize(isfirefox,platformos)
{
    ...
    function(local)
    {
        ...
        if (!("options-urllisttime" in local)) local["options-urllisttime"] = 10;
        ...
        function(tabs)
        {
            ...
            if (local["options-useautomation"])
            {
                ...
                else
                {
                    ...
                    {
                        if (tab.id == tabs[0].id && tab.status == "complete")
                        {
...
function(highlightInfo)
{
    ...
    function(local)
    {
        chrome.storage.local.set({ "tabs-highlightcount": highlightInfo.tabIds.length });       
...
function initiateAction(menuaction,saveditems,togglelazy,extractsrcurl,externalsave,swapdevices,local)
{
    ...
    function(tabs)
    {
        ...
        if (menuaction == 0)
        {
            ...
            {
                if (tabs[i].highlighted || tabs[i].active || local["options-useautomation"]) selectedtabids.push(tabs[i].id);  /* Opera doesn't support highlighted - so check active */
            }
            
            multiplesaves = (selectedtabids.length > 1);
            
            chrome.storage.local.set({ "actions-cancelsave": false },
            function()
            {
        ...
        else if (menuaction == 1)
        {
            ...
            {
                listedurls.push(local["options-urllisturls"][i]);
            }
...
function nextAction(menuaction,saveditems,togglelazy,extractsrcurl,externalsave,swapdevices,windowid,selectedtabids,listedurls,multiplesaves,local)
{
    ...
    function(local)
    {
        ...
        if (menuaction == 0)
        {
            ...
            else if (local["options-useautomation"]) 
            {
                setTimeout(  /* allow time to catch Alt+C after saving all tabs */
                function()
                {
                    if (!local["actions-cancelsave"])
                    {
                        ...
                        function(win)
                        {
                            chrome.windows.remove(win.id);  /* remove window created for automation */
                        });
                    }
                },1000);
            }
        ...
        else if (menuaction == 1)
        {
            if (listedurls.length > 0)
            {
                ...
                function()
                {
                    ...
                    nextAction(menuaction,saveditems,togglelazy,extractsrcurl,externalsave,swapdevices,windowid,selectedtabids,listedurls,multiplesaves,local);
                },local["options-urllisttime"]*1000);
...
function finishAction(tabId,success,local)
{
    ...
    {
        chrome.tabs.remove(tabId);  /* remove tab created for saving listed URL */
    }
    ...
    chrome.storage.local.set({ "actions-cancelsave": false, "actions-details": local["actions-details"] },
    function()
    {
        nextAction(menuaction,saveditems,togglelazy,extractsrcurl,externalsave,swapdevices,windowid,selectedtabids,listedurls,multiplesaves,local);
    });
...
{
    chrome.storage.local.set({ "actions-cancelsave": true },
    function()
    {
        var tabId;
...
{
    chrome.notifications.create("alert",{ type: "basic", iconUrl: "icon32.png", title: "SAVE PAGE VIP", message: "" + message });
}
